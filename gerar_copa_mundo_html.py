...
def _load_model_adjustments() -> dict[str, object]:
    ...
    normalized = {
        "draw_multiplier": round(_clamp(float(adjustments.get("draw_multiplier", 1.0)), 0.78, 1.48), 3),
        "home_goal_multiplier": round(_clamp(float(adjustments.get("home_goal_multiplier", 1.0)), 0.72, 1.34), 3),
        "away_goal_multiplier": round(_clamp(float(adjustments.get("away_goal_multiplier", 1.0)), 0.72, 1.34), 3),
        "home_advantage_points": round(_clamp(float(adjustments.get("home_advantage_points", 2.3)), 0.0, 6.0), 2),
        "confidence_bias": round(_clamp(float(adjustments.get("confidence_bias", 0.0)), -12.0, 12.0), 2),
        "team_rating_adjustments": {},
    }

    raw_team_adjustments = adjustments.get("team_rating_adjustments", {})
        ...
        <div class="model-grid">
          <div class="model-card model-card-wide">
            ...
            <p>
              ...
              Se quiser ajustar manualmente a leitura, edite o arquivo <code>{escape(MODEL_ADJUSTMENTS_FILE.name)}</code> e gere a pagina novamente.
            </p>
          </div>
